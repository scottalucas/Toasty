...
//        testAzAcct = try! AmazonAccount(with: prof)!.save(on: db).wait()
//        goodTestFp1 = try! Fireplace(power: .ok, imp: "https://agent.electricimp.com/2arZveArVIRJ", id: "test", friendly: "test 1").save(on: db).wait()
//        goodTestFp2 = try! Fireplace(power: .ok, imp: "https://agent.electricimp.com/7NjKoDqiOxi5", id: "test", friendly: "test 1").save(on: db).wait()
//        badUrltestFp1 = try! Fireplace(power: .line, imp: "https://httpstat.us/200?sleep=8000", id: "test", friendly: "test 2").save(on: db).wait()
...
////        let expectedFps = try! AlexaController.getAssociatedFireplaces().wait()
