        ...
        func powerControllerHandler(_ req: Request) throws ->
            Future<Response> {
                guard
                    ...
                    let action:ImpFireplaceDirective = ImpFireplaceDirective(rawValue: inboundMessage.directive.header.name),
                ...
                return associatedFireplaces
                    .flatMap(to: ImpStatus.self) { fireplaces in
                        let optFireplace = fireplaces
                            .filter ({return (($0.id != nil) ? $0.id! == targetFireplaceId : false)})
                            .first
                        guard let fireplace = optFireplace else {return Future.map(on: req) {ImpStatus.NotAvailable}}
                        return FireplaceManagementController.action(action, executeOn: fireplace.controlUrl, on: req)
                    }.flatMap (to: Response.self) { result in
                        return try AlexaFireplaceDirectiveResponse(impStatus: result, msgId: messageId, corrToken: correlationToken, accToken: userRequestToken, endptId: endpoint.endpointId, context: req)
                    }
        }
...
func AlexaFireplaceDirectiveResponse (impStatus: ImpStatus, msgId: String, corrToken: String?, accToken: String, endptId: String, context req: Request) throws -> Future<Response> {
    switch impStatus {
    case .OFF, .ON:
        let props = [ AlexaProperty(namespace: .power, name: "powerState", value: impStatus.rawValue, time: Date(), uncertainty: 500) ]
